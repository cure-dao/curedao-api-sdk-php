...
use CureDAO\Client\Computers\ThisComputer;
use CureDAO\Client\Exceptions\InvalidFilePathException;
use CureDAO\Client\Exceptions\InvalidStringException;
use CureDAO\Client\Exceptions\InvalidUrlException;
...
use CureDAO\Client\Slim\Model\User\QMUser;
use CureDAO\Client\Traits\QMValidatingTrait;
use CureDAO\Client\UI\HtmlHelper;
use CureDAO\Client\Utils\APIHelper;
use CureDAO\Client\Utils\AppMode;
use CureDAO\Client\Utils\QMAPIValidator;
...
";
    ...
    public static function validateUrl(string $url, string $type){
        QMValidatingTrait::assertIsUrl($url, $type);
    }
    /**
     * @param string $string
     ...
     * @return string
     */
    public static function truncate(string $string, int $length = 15, string $warning = "..."): string{
    ...
    public static function outputSnakeCamelCaseMapsForObject($obj): void{
        foreach($obj as $camel => $value){
            ...
            \CureDAO\Client\Logging\ConsoleLog::info("'$snake' => '$camel',");
        }
        foreach($obj as $camel => $value){
            $snake = QMStr::snakize($camel);
            \CureDAO\Client\Logging\ConsoleLog::info("'$camel' => '$snake',");
        }
    }
    /**
     * @param string $input
    ...
    public static function assertShorterThan(int $max, string $str, string $type){
        QMValidatingTrait::assertStringShorterThan($max, $str, $type);
    }
    /**
    ...
    public static function assertContains(string $haystack, $requiredStrings, string $type, bool $ignoreCase = false,
                                          string $message = null){
        QMValidatingTrait::assertStringContains($haystack, $requiredStrings, $type, $ignoreCase, $message);
    }
    /**
     * @param string $haystack
     * @param $blackList
    ...
    public static function assertDoesNotContain(string $haystack, $blackList, string $type, bool $ignoreCase = false,
        ...
        QMValidatingTrait::assertStringDoesNotContain($haystack, $blackList, $type, $ignoreCase, $assertionMessage);
    }
    /**
     * @param int $expected
     * @param string $needle
    ...
    public static function assertIsString($val){
        if(!is_string($val)){
            le("should be string but is " . print_r($val, true));
        }
    }
    /**
     * @param int $length
...
{", " {", $code);
    ...
    public static function printNotNullOrEmptyStringValues($arrOrObj): string{
        return print_r(QMArr::notEmptyValues($arrOrObj), true);
    }
    /**
     * @param array|object $arrOrObj
     * @return bool|string
    ...
    public static function printStringsAndNumbers($arrOrObj): string{
        return print_r(QMArr::getStringsAndNumbers($arrOrObj), true);
    }
    /**
